...
"fenodes"="fe.doris.svc.cluster.local:8030",
"user"="root",
"password"=""
)
;
...
EOF
  ...
  kubectl logs -n spark-operator `kubectl get pod -n spark-operator | grep ${jobname} | awk '{print $1}'` > driver-${tbl}-${logfile}  2>&1
  kubectl exec -it -n doris `kubectl get pod -n doris | grep Running | grep doris-fe-0 | awk '{print $1}'` -- \
    mysql --default-character-set=utf8 -h fe -P 9030 -u'root' -e "USE ${db};SELECT COUNT(1) FROM ${tbl};"
done
...
kubectl get pod -n spark-operator|grep tmp-mpp-doris-ingestion
kubectl get pod -n spark-operator|grep tmp-mpp-doris-ingestion |grep 'Error' |awk '{print $1}'| xargs kubectl delete pod "$1" -n spark-operator --force --grace-period=0
kubectl get pod -n spark-operator|grep tmp-mpp-doris-ingestion |grep -v 'Running' |awk '{print $1}'| xargs kubectl delete pod "$1" -n spark-operator --force --grace-period=0
kubectl get pod -n spark-operator|grep tmp-mpp-doris-ingestion |grep -v 'Running\|ContainerCreating\Pending' |awk '{print $1}'| xargs kubectl delete pod "$1" -n spark-operator --force --grace-period=0

kubectl logs -n spark-operator `kubectl get pod -n spark-operator|grep tmp-mpp-doris-ingestion |grep 'Error' |awk '{print $1}'`
