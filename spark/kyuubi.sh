...
helm install kyuubi -n spark-operator -f values.yaml \
...
helm uninstall kyuubi -n spark-operator
...
kubectl get pod -n spark-operator |grep Running | grep kyuubi | awk '{print $1}'| xargs kubectl delete pod "$1" -n spark-operator --force --grace-period=0

#kubectl logs -n spark-operator kyuubi-server-
kubectl logs -n spark-operator `kubectl get pod -n spark-operator | grep Running | grep kyuubi | awk '{print $1}'`
...
kubectl exec -it -n spark-operator `kubectl get pod -n spark-operator | grep Running | grep kyuubi | awk '{print $1}'` -- bash

kubectl exec -it -n spark-operator `kubectl get pod -n spark-operator | grep Running | grep kyuubi | awk '{print $1}'` -- cat conf/kyuubi-defaults.conf
...
kubectl exec -it -n spark-operator `kubectl get pod -n spark-operator | grep Running | grep spark-test | awk '{print $1}'` -- bash
  ...
  beeline -u jdbc:hive2://kyuubi-thrift-binary.spark-operator.svc.cluster.local:10009 -n hdfs
    SHOW DATABASES;
