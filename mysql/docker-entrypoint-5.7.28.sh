...
_exec_sql_file_with_pwd_replaced_only_once() {
	...
	if [[ -f ${DATADIR}/"$filename".success ]]; then 
		...
		docker_process_sql < /tmp/${filename} > ${DATADIR}/$file.log 2>&1
		if [[ $? == 0 ]]; then
			touch ${DATADIR}/$file.success
		fi
		rm -f /tmp/${filename}
		cat ${DATADIR}/$file.log
	fi
}

#function _exec_sql_file_with_pwd_replaced_only_once is added for both replacing secured password and avoid duplicated initialization
_exec_sql_file_with_pwd_replaced_only_once() {
	local file="$1"
    echo "file:${file}"
	local filename=`basename ${file}`
...
docker_process_init_files() {
	...
	#line *.sql is modified to avoid sql executed every time container is restarted
	echo
	local f
	for f; do
		case "$f" in
